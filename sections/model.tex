...
Where $\phi$ is given in degrees and $z$ in meters above contemporary sea level. This models weakening of the till associated with the presence of marine sediments. Basal topography (Fig.~\ref{fig:topo}) is derived from the ETOPO1 combined topography and bathymetry dataset \citep{data:etopo1}.\julien{NaturalEarth data are vectors (coastline, rivers, etc.) used in Fig. 1, not model input.} It responds to ice load following a regional isostasy model that includes lithospheric flexure and mantle relaxation \citep{lingle-clark-1985}.
