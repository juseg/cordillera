...
for i, (conf, gflx) in enumerate(zip(confargs, gflxargs)):
    ...
    # plot
    im = nc.imshow('bmelt', ax, cmap='Reds', vmin=0.0, vmax=0.2)
    nc.contourf('bmelt', ax, levels=[-1e6, 1e-6], colors='none', hatches=['//'])
    nc.contour('bmelt', ax, levels=[1e-6], colors='k', linewidths=0.25)
