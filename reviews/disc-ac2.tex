...
Through the till friction angle parameter $\phi$, sliding is controlled by bedrock elevation. This control is meant to reflect the presence of lubricating marine sediment at the ice sheet base. Such sediments are expected below current sea level, and, to a lesser extent, in areas that were submerged during former glacial periods, which is reflected in our simulation set-up by a linear increase of $\phi$ between 0 and 200 meters above sea level, where it reaches its maximum value.

We want to clarify here that $\phi$ is computed only once and for the entire simulation length at the beginning of the run, based on modern bedrock elevation. Because the distribution of marine sediments is not likely to change much during a single phase of ice sheet advance and retreat, sliding is not affected by bedrock elevation changes due to isostatic effects in our runs.
