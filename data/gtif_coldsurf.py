...
for ghf in ghfmaps:
    ...
    for i in indexes:
        ...
        h = nc.variables['tempicethk_basal'][i]

        # compute transition surface altitude
        z = b + h

        # generate geotiff
        ofilepath = '%s-coldsurf-%da.tif' % (prefix, age)
    ...
    # create zip archive
    with zipfile.ZipFile(prefix + '-coldsurf.zip', 'w') as zf:
        for f in ofilelist:
