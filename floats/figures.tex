...
\begin{figure}[t]
	...
	\end{center}
    \irina[inline]{include all geographic names}
    \arjen[inline]{elevation scale white-end confused with glaciers}
    \arjen[inline]{remove Arctic circle and miles scale}
    \arjen[inline]{use CIS, LIS and IIS for ice sheet names}
    \arjen[inline]{use N, S, E and W instead of minus, add 75\degree N label}
...
\begin{figure*}[t]
	...
	\end{center}
	\caption{Ice surface topography (black contours every 1000\,m) and velocity (\unit{m\,yr^{-1}}) after 10\,kyr under a climate 5\degC colder than present for each climate forcing. Drawn using Matplotlib \citep{soft:mpl}.}
	\label{fig:cool05}
\end{figure*}
