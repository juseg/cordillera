...
\def\note#1{\textcolor[rgb]{0,0,0.5}{\textbf{[NOTE: #1]}}}
...
deglaciation phase and a regional level, while little is known about the ice
...
patters are available for the Laurentide and Eurasian ice sheets, such is not
the case for the Cordilleran ice sheet, where complex patterns of ice flow
...
Temperature offset time-series are reconstructed from proxy records from
...
\todo{add equation here,}

while temperature reconstructions from the Antarctic and oceanic cores was
provided by their authors. In addition, all records were scaled linearly in
...
topography and specific to each climate forcing dataset.
...
Fig.~\ref{fig:timeseries}). A ice volume minimum is attained in between these
two events during MIS~3 (52.2--45.6\,\unit{kyr}).

On the other hand, the magitude and timing of these events vary significantly
between our simulations. For instance, GRIP and NGRIP forcing result in highest
variability in modelled ice volume, while simulations driven by oceanic records
and the NGRIP reach last glacial maximum ice volume prior to 20.0\,\unit{kyr}.
...
this ice cap, as well as the magnitude of the MIS~4, depends on the temperature
