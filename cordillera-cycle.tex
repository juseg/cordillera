...
\end{itemize}
      ...
      these climate forcing time-series and to some of the most influential
      ice flow parameters, we then proceed to compare the model output
...
\label{sec:overview}%
      ...
      3\,\unit{km} depth (Sect.~\ref{sec:icedyn}). Basal sliding follows
      ...
      (Sect.~\ref{sec:atm}, Table~\ref{tab:records}).
...
\end{align}
%
      where $\vec{\dot{\epsilon}}$ is the the strain-rate tensor,
...
%
\begin{itemize}
  \item Our \emph{default} configuration used in the control run and all
    other simulations in the manuscript include rheological parameters,
...
\end{align}
      ...
      modern sea level (0\,\unit{m} above sea level, \unit{m\,a.s.l.}) and
...
\caption{%
      ...
      $W$, for the default ($\delta=0.02$, $W_{max}=2$\,m), hard bed
      ($\delta=0.05$, $W_{max}=5$\,m), and soft bed ($\delta=0.01$,
      $W_{max}=1$\,m) sliding parametrisations, using a linear scale (top
      panel) and a logarithmic scale (bottom panel). Calculations are made
      for an ice thickness, $h$, of 1000\,m. Figure made using
